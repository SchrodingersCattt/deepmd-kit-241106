    ...
    **kwargs : dict
    ...
    def eval_descriptor(
        ...
        **kwargs: Dict[str, Any],
        ...
        """Evaluate the descriptor by using this DP.
        ...
        **kwargs
        ...
        if "spin" not in kwargs or kwargs["spin"] is None:
            descriptor = self._eval_func(self._eval_descriptor_inner, numb_test, natoms)(
                coords, cells, atom_types, fparam, aparam
            )
        else:
            raise RuntimeError("DeepSpin does not support inference descriptor now!")
        return descriptor
    ...
    def _eval_descriptor_inner(
        ...
        cc, bb, fp, ap, input_prec = self.input_type_cast(
            coord_input, box=box_input, fparam=fparam_input, aparam=aparam_input
        )
        del coord_input, box_input, fparam_input, aparam_input
        (
            extended_coord,
            extended_atype,
            mapping,
            ...
            cc,
            type_input,
            self.dp.model["Default"].atomic_model.get_rcut(),
            self.dp.model["Default"].atomic_model.get_sel(),
            self.dp.model["Default"].atomic_model.mixed_types(),
            box=bb,
        )
        nframes, nall = extended_atype.shape[:2]
        extended_coord = extended_coord.view(nframes, -1, 3)
        nlist = self.format_nlist(extended_coord, extended_atype, nlist)
        cc_ext, _, fpp, app, input_prec = self.input_type_cast(
            extended_coord, fparam=fp, aparam=ap
        )
        del extended_coord, fp, ap, fpp, app
        descriptor, p1, p2, p3, p4 = self.dp.model["Default"].atomic_model.descriptor(
            cc_ext,
            extended_atype,
            nlist,
            mapping=mapping,
        ...
        return descriptor.cpu().detach().numpy()
    ...
    def format_nlist(
        ...
        """
        mixed_types = self.dp.model["Default"].atomic_model.mixed_types()
        nlist = self._format_nlist(extended_coord, nlist, sum(self.dp.model["Default"].atomic_model.get_sel()))
        if not mixed_types:
            nlist = nlist_distinguish_types(nlist, extended_atype, self.dp.model["Default"].atomic_model.get_sel())
        return nlist
    ...
    def _format_nlist(
        ...
        rcut = self.dp.model["Default"].atomic_model.get_rcut()
    ...
    def input_type_cast(
        ...
        input_prec = RESERVED_PRECISON_DICT[coord.dtype]
        ###
