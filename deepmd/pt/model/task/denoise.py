...
class DenoiseFittingNet(Fitting):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.old_impl:
            self.pair2coord_proj = NonLinearHead(in_dim_coord, 1, activation_fn=self.activation_function)
            self.lm_head = MaskLMHead(
                embed_dim=in_dim_logits,
                output_dim=net_dim_out[1],
    ...
    def forward(
        ...
        if self.old_impl:
            if self.mixed_types:
                outs_logits += self.lm_head(xx)
                outs_coord += ((self.pair2coord_proj(g2)) * h2).sum(dim=-2) / (sw.sum(dim=-1).unsqueeze(-1) + 1e-6)
            else:
                raise RuntimeError("Denoise is only supported in mixed_types situation!")
        else:
            if self.mixed_types:
                outs_coord += ((self.filter_layers_coord.networks[0](g2)) * h2).sum(
                    dim=-2
                ) / (sw.sum(dim=-1).unsqueeze(-1) + 1e-6)  # Shape is [nframes, natoms[0], net_dim_out]
                outs_logits += self.filter_layers_logits[0](xx)
            else:
                raise RuntimeError("Denoise is only supported in mixed_types situation!")
